...
#| label: fig-surge-slr
#| code-fold: true
#| fig-cap: "Left: the fixed Norfolk-calibrated GEV annual-max storm surge distribution (PDF, in feet). Right: polynomial MSL trajectories $z_t = a + b t + c t^2$ for 20 representative scenarios (feet above baseline)."
let
...
```{julia}
#| label: fig-static-grid
#| code-fold: true
#| fig-cap: "Static heightening grid. Each point is a static one-shot heightening; the red diamonds mark the non-dominated subset (the static Pareto front). Dashed curves are iso-cost reference lines (investment + damage = constant; curved because of the pseudo-log axes)."
let
...
```{julia}
#| label: fig-adaptive-grid
#| code-fold: true
#| fig-cap: "Brute-force grid over the (buffer, freeboard) policy space. Each point is one adaptive policy; red diamonds mark the non-dominated set. The static non-dominated points are shown in orange for reference."
let
    fig = Figure(; size=(650, 650))
...
#| code-fold: true
#| fig-cap: "NSGA-II adaptive Pareto front (blue circles + line), with the static non-dominated policies (orange triangles) and the brute-force adaptive grid's non-dominated set (red diamonds) for comparison."
# Your code here
```
...
#| code-fold: true
#| fig-cap: "Evolution of the adaptive Pareto front over NSGA-II generations. For each generation we plot only that generation's non-dominated set (not the full population), so the lines reveal how the front migrates toward the bottom-left over time."
# Your code here
```
